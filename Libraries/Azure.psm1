	...
	try	{
			...
			foreach ($item in $currentStatus) {
				if ($item.Name.Value -eq $text) {
					...
					if ($currentStatus[$counter].CurrentValue -le $allowedCount) {
						LogMsg "  Current $text Estimated use: $($currentStatus[$counter].CurrentValue)"
					} else {
				...
				if ($item.Name.Value -eq "cores") {
					$allowedCount = [int](($currentStatus[$counter].Limit)*($AllowedUsagePercentage/100))
					if ($currentStatus[$counter].CurrentValue -le $allowedCount) {
						LogMsg "  Current Regional Cores Estimated use: $($currentStatus[$counter].CurrentValue)"
					} else {
		...
		foreach ($VM in $RGXMLData.VirtualMachine) {
			...
			if (($OverrideVMSize -or $CurrentTestData.OverrideVMSize) -and ($testVMUsage -gt 0)) {
				LogMsg "Test VM or Override size VM"
			} else {
...
#			$xRegionTotalLocations = $location.Split("-").Count
		...
		foreach ($RG in $setupTypeData.ResourceGroup ) {
			...
			while (!$readyToDeploy) {
				...
				if ( $elapsedWaitTime -gt $CoreCountExceededTimeout ) { break }
			}
...
Function DeleteResourceGroup([string]$RGName, [switch]$KeepDisks) {
	try	{
		...
		LogErr "Can not find Azure Resource Group info"
	}
	...
	if ($ResourceGroup) {
		if ($ExistingRG) {
			...
			if ($CleanupRG)	{
				...
				Start-Job -ScriptBlock $cleanupRGScriptBlock -ArgumentList $RGName,$currentGUID -Name "DeleteResourceGroup-$RGName"
				$retValue = $true
			}
		}
	} else {
...
Function GenerateAzureDeployJSONFile ($RGName, $osImage, $osVHD, $RGXMLData, $Location, $azuredeployJSONFilePath) {
	...
	if ( $StorageAccountName -inotmatch "NewStorage" -and !$UseManagedDisks) {
		...
		#$StorageAccountRG = ($GetAzureRMStorageAccount | Where-Object {$_.StorageAccountName -eq $StorageAccountName}).ResourceGroupName.ToString()
...
Function CopyVHDToAnotherStorageAccount ($sourceStorageAccount,$sourceStorageContainer,$destinationStorageAccount,$destinationStorageContainer,$vhdName,$destVHDName)
{
	...
	Start-AzureStorageBlobCopy -AbsoluteUri $SasUrl  -DestContainer $destContainer -DestContext $destContext -DestBlob $destBlob -Force
	#
